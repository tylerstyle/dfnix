...
2. **Tool Modifications**:
   - Changes to forensic tools (`dfdisk`, `dfmount`, `dfnet`) must be made and tested in `~/git/<tool>`, not solely inside `dfnix/pkgs/`.
   - Ensure companion scripts in `dfnix/pkgs/<tool>` are synced with `~/git/<tool>/scripts/`.
3. **Verification**:
